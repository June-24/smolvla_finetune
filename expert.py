...
expert.py
=========
SmolVLMWithExpertModel — VLM backbone + smaller action-expert transformer.

No lerobot dependencies. Pure PyTorch + HuggingFace transformers.
"""
...
def apply_rope(x, positions, max_wavelength=10_000):
    """Apply RoPE positional encoding. positions: [B, L], x: [B, L, H, D]."""
    d_half  = x.shape[-1] // 2
    device  = x.device
    dtype   = x.dtype
...
class SmolVLMWithExpertModel(nn.Module):
    def __init__(
        ...
        if num_expert_layers > 0:
            assert len(self.get_vlm_model().text_model.layers) % num_expert_layers == 0, (
                f"VLM layers ({len(self.get_vlm_model().text_model.layers)}) "
                f"must be a multiple of num_expert_layers ({num_expert_layers})"
            )
        ...
        if "cross" in attention_mode:
            # Reshape k/v projections so their input dim matches the VLM hidden size
    ...
    def set_requires_grad(self):
        ...
        if self.train_expert_only:
            ...
            frozen_names = ["lm_head", "text_model.model.norm.weight"]
            for layer in last_layers:
                frozen_names.append(f"text_model.model.layers.{layer}.")
            for name, p in self.vlm.named_parameters():
                if any(k in name for k in frozen_names):
                    p.requires_grad = False

        # Avoid unused-param issues in distributed training
    ...
    def forward_cross_attn_layer(
        ...
        if len(inputs_embeds) == 2 and not past_key_values:
            ...
            att_output = self.eager_attention_forward(
                prefix_attention_mask, batch_size, head_dim,
                query_states, key_states, value_states
            )
        ...
        # Expert cross-attends to VLM KV cache
    ...
    def eager_attention_forward(
        self, attention_mask, batch_size, head_dim,
        query_states, key_states, value_states
    ):
        num_att_heads      = self.num_attention_heads
        num_key_value_heads = self.num_key_value_heads
        num_kv_groups      = num_att_heads // num_key_value_heads
        seq_len            = key_states.shape[1]

        key_states = key_states[:, :, :, None, :].expand(
            batch_size, seq_len, num_key_value_heads, num_kv_groups, head_dim
